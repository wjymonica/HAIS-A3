...
def crop(img, center, scale, output_size, rot=0):
    ...
    if sf < 2:
        ...
        if new_size < 2:
            ...
            #print(img)
            img = np.array(Image.fromarray((img* 255).astype(np.uint8)).resize(size=(new_ht, new_wd)))
    ...
    #print(new_img)
    new_img = np.array(Image.fromarray(new_img.astype(np.uint8)).resize(size=output_size))
    return new_img
